...
let filterData = { season: 2023, from: "2023-08-11", to: "2023-08-21" };
let searchParams = getSearchParams(filterData);
let Url = `https://v3.football.api-sports.io/fixtures?league=39&timezone=Africa/Cairo${
...
function renderMatches(matches) {
  ...
  for (let i = 0; i < matches.length; i++) {
    let match = {
      ...
      date: formatDateTimeArabic(matches[i].fixture.date),
    };
    const matchCard = `
...
$("#filter-btn").on("click", (e) => {
  ...
  const from = $(".filters #dateFrom").val() || "2023-08-11";
  const to = $(".filters #dateTo").val() || "2023-08-21";
  ...
  getMatches(
    `https://v3.football.api-sports.io/fixtures?${searchParams}&league=39&timezone=Africa/Cairo`
  );
});
